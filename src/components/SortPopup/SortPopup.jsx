...
const SortPopup = React.memo(function SortPopup({ items, activeSortType, onClickSortType }) {
  ...
  const activeName = items.find((el) => el.type === activeSortType).name;
    ...
    >
      ...
      {visiblePopup && (
        <div className="sort__popup">
          <ul>
            {items.map((el, index) =>
              (<li onClick={() => onSelectItem(el)} className={activeSortType === el.type ? 'active' : ''} key={`${el.type}_${index}`}>{el.name}</li>))}
          </ul>
        </div>
      )}
    </div>
